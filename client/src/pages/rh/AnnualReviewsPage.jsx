...
import AnnualReviewForm from '../../components/rh/AnnualReviewForm';

const STATUS_CONFIG = {
    'UP_TO_DATE': { label: 'Réalisé', color: 'green', icon: CheckCircle2 },
    'UPCOMING': { label: 'À planifier', color: 'navy', icon: Clock },
...
export default function AnnualReviewsPage() {
    ...
    const fetchReviews = async () => {
        try {
            setLoading(true);
            const { data } = await axios.get('/compliance/annual-reviews');
            setUsers(data.users);
        } catch (error) {
        ...
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Entretiens Annuels</h1>
                        </div>
                        <p className="text-gray-500 font-medium">Bilan de performance et de développement annuel.</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                    ...
                    { label: 'À Planifier', value: users.filter(u => u.status === 'UPCOMING').length, icon: Clock, color: 'navy' },
                    { label: 'Réalisés', value: users.filter(u => u.status === 'UP_TO_DATE').length, icon: CheckCircle2, color: 'green' },
                ].map((stat, i) => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            <option value="UP_TO_DATE">Réalisé</option>
                    ...
                    <table className="min-w-full divide-y divide-gray-100">
                        <thead className="bg-gray-50/50">
                            <tr>
                                ...
                                <th className="px-8 py-5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Date Dernier EADP</th>
                                <th className="px-8 py-5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Prochaine Échéance</th>
                                ...
                                return (
                                    <tr key={user.id} className="hover:bg-gray-50/50 transition-colors group">
                                        ...
                                        <td className="px-8 py-6 whitespace-nowrap text-sm text-gray-500 font-bold">
                                            {user.last_annual_review ? new Date(user.last_annual_review).toLocaleDateString() : 'Jamais'}
                                        </td>
                                        <td className="px-8 py-6 whitespace-nowrap text-sm text-gray-500 font-bold">
                                        ...
                                        <td className="px-8 py-6 whitespace-nowrap">
                                            <div className={clsx(
                                                ...
                                                cfg.color === 'navy' && "bg-navy-50 text-navy-700",
            ...
            {selectedUserId && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8">
                    <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-sm" onClick={() => setSelectedUserId(null)}></div>
